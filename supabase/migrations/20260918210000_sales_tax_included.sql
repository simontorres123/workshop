-- Las ventas reciben precios finales (con IVA incluido) desde el inventario.
-- p_tax conserva únicamente el importe desglosado; nunca se suma de nuevo al total.
...
BEGIN
  ...
  -- El precio de inventario ya es el total final. p_tax solo documenta su desglose.
  v_total := GREATEST(v_subtotal - p_discount, 0);
  UPDATE workshop_sales
  SET subtotal = v_subtotal,
      total = v_total,
      change_amount = GREATEST(p_amount_paid - v_total, 0),
      status = CASE WHEN p_amount_paid >= v_total THEN 'paid' ELSE 'pending' END,
      updated_at = NOW()
  WHERE id = v_sale.id
  RETURNING * INTO v_sale;
  RETURN v_sale;
