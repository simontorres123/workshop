...
BEGIN
  ...
  LOOP
    ...
    -- Determinar Link (CORRECCIÓN: las rutas no llevan /admin/ en la URL)
    v_link := CASE r.type
      WHEN 'warranty_expiring' THEN '/inventory?filter=warranty_expiring'
      WHEN 'low_stock' THEN '/inventory?filter=low_stock'
      WHEN 'pending_repairs' THEN '/repairs?status=pending'
      WHEN 'daily_summary' THEN '/dashboard'
      WHEN 'backup_reminder' THEN '/system'
      ELSE '/dashboard'
    END;

    -- 1. Insertar notificación en in_app_notifications
