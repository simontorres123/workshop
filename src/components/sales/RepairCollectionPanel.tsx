...
type RepairBilling = { id: string; folio: string; branchId: string; clientName: string; clientPhone?: string; device: string; completedAt?: string; assignedTechnicianId?: string | null; laborCost: number; parts: RepairPart[]; estimatedTotal: number; existingSale?: { saleNumber: string; total: number } | null };
...
export default function RepairCollectionPanel({ taxRate, onSaleCreated }: { taxRate: number; onSaleCreated: () => Promise<void> | void }) {
  ...
  const selectedAvailable = selected && !selected.existingSale && Boolean(selected.assignedTechnicianId);
  ...
  return <Stack spacing={2}>
    ...
    <Grid container spacing={2}>{repairs.map(repair => <Grid key={repair.id} size={{ xs: 12, md: 6 }}><Card variant="outlined" sx={{ height: '100%', borderColor: selected?.id === repair.id ? 'primary.main' : 'divider' }}><CardContent><Stack direction="row" justifyContent="space-between" gap={1}><Box><Typography fontWeight={800}>{repair.folio}</Typography><Typography variant="body2">{repair.clientName}</Typography><Typography variant="body2" color="text.secondary">{repair.device}</Typography></Box><Chip size="small" color={repair.existingSale ? 'success' : repair.assignedTechnicianId ? 'warning' : 'error'} label={repair.existingSale ? 'Cobro registrado' : repair.assignedTechnicianId ? 'Lista para cobrar' : 'Sin técnico'} /></Stack><Stack direction="row" justifyContent="space-between" sx={{ mt: 2 }}><Typography color="text.secondary">Total estimado</Typography><Typography fontWeight={800}>{money(repair.estimatedTotal)}</Typography></Stack>{!repair.assignedTechnicianId && <Alert severity="warning" sx={{ mt: 1.5, py: 0 }}>Asigna un técnico en Reparaciones antes de cobrar.</Alert>}<Button fullWidth sx={{ mt: 1.5 }} variant={selected?.id === repair.id ? 'contained' : 'outlined'} disabled={Boolean(repair.existingSale) || !repair.assignedTechnicianId} onClick={() => setSelected(repair)}>{repair.existingSale ? `Cobrado ${money(repair.existingSale.total)}` : repair.assignedTechnicianId ? 'Revisar y cobrar' : 'Requiere técnico'}</Button></CardContent></Card></Grid>)}</Grid>
